...
class Dice:                 # 10번 문제 클래스 
    def __init__(self):     # numbers라는 빈 리스트 생성 
        self.numbers = []   

    def playDice(self):     # 주사위 던지는 행위 (1에서 6사이의 랜덤값을 생성하여 numbers 리스트에 append) 
        self.numbers.append(random.randint(1,6))

    def getNumbers(self):   # numbers 리스트에 있는 내용 반환 
        return self.numbers

    def getSum(self):       # numbers 리스트에 있는 값들의 합을 반환
